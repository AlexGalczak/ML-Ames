...
house_price %>% 
  ggplot(aes(x = BsmtFinPerc, y=SalePrice, color = SalePrice)) + 
  geom_point() + 
  facet_grid(~AgeBin) +
  labs(title = "Price by Percent Finished Basement Grouped by House Age", x = "Total Percentage of Basement Finished", y="Sale Price (Thousand Dollars)")
```
...
bedlevels = df %>% filter(nghbr_Blueste == 0) %>% .$BedroomAbvGr
df %>% mutate(BedroomAbvGr=factor(BedroomAbvGr, bedlevels)) %>%
