...
house_price %>% 
  ggplot(aes(x = BsmtFinPerc, y=SalePriceShort, color = SalePrice)) + 
  facet_grid(.~AgeBin, scales='free_x', space = 'free_x') +
  geom_point() + 
  labs(title = "Price by Percent Finished Basement Grouped by House Age", x = "Total % of Basement Finished", y="Sale Price ($1000 USD)") +
  theme_bw() +
  theme_ipsum() +
  theme(axis.text.x=element_text(size=11, angle=90, vjust=.8, hjust=0.8))
...
house_price %>% 
  ggplot(aes(x=Age, y=SalePrice, colour = Foundation, alpha=0.6)) +
  geom_point() +
  theme_bw() + 
  theme_ipsum() + 
  scale_alpha(guide = "none") + 
  labs(title = 'Year Built by Sale Price Grouped by Foundation', x = 'House Age', y='Sale Price') + 
  scale_fill_manual(values=c("#264D59", "#138086","#43978D", "#63C0B5", "#90D1C9"))
...
house_price %>% 
  ggplot(aes(x=Age, y=SalePrice, colour = MasVnrType, alpha=0.6)) +
  geom_point() +
  theme_bw() + 
  theme_ipsum() + 
  scale_alpha(guide = "none") + 
